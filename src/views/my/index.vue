<template>
  <div>
    ...
    >
      <img
        class="h-22 w-22 border-2 border-solid !absolute -top-10 rounded-50%"
      ...
      <van-cell title="谷歌验证" :value="googleAuth" />
      <van-cell title="上次登录IP" :value="lastLoginIp" />
      <van-cell title="上次登录时间" :value="lastLoginTime" />
...
const { nickname, roleName, lastLoginIp, lastLoginTime, googleAuth } = userStore.getUserInfo;
...
.my-bg {
  ...
  &::after {
    ...
    width: 100%;
    height: 100%;
    background-image: linear-gradient(180deg, rgba(0, 0, 0, 0), #548de5);
    // background-image: url('../../assets/myBg.gif.gif');
    opacity: 0.9;
  }
}
