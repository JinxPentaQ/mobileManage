  ...
  <div class="flex flex-col h-dvh overflow-hidden">
    ...
    >
      <div v-for="(item, index) in list" :key="index">
        <CollectionItem :item="item" @click="onDetail(item.id)" />
      </div>
    </van-list>
    <CollectionFilter
...
import { collectionStatusList } from "@/views/order/utils";
...
const typeOptions = ref([{ text: "全部状态", value: "" }, ...collectionStatusList.value]);
...
function getList() {
  ...
  CollectOrderStoreHook()
    .getOrderList(requestParams.value)
    .then((res) => {
      console.log(res);
      if (res.code === "200") {
...
function onDetail(id) {
  router.push({ name: "CollectionDetail", query: { id: id } });
}
